...
В структурированном ответе модели отдельное предложение с пустым объяснением (включая пробелы), заголовком или рекомендацией отбрасывается с записью в `warnings`. Для отклонённого сопоставления сохраняется исходный детерминированный кандидат; остальные допустимые предложения модели применяются. Пустая необязательная аналитическая записка остаётся `null`. Это не подмена режима: результат сохраняет `analysis_mode=semantic` и сообщает об отклонённых предложениях.
