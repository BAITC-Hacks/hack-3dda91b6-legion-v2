...
- Успех: HTTP 2xx и **сам объект CORE `AnalysisResult`** из [backend/schemas.py](backend/schemas.py), без оболочки `data`. Асинхронные job ID, polling и streaming не поддерживаются. Таймаут frontend — 270 секунд; сброс прекращает ожидание и отменяет клиентский запрос, но не гарантирует остановку серверного вычисления.
