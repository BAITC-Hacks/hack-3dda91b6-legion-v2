...
Источник истины: [src/types.ts](src/types.ts) и `validateAnalysisResponse` в [src/services/analysis.ts](src/services/analysis.ts). Полный согласованный пример — [src/data/demo.json](src/data/demo.json); все его документы и цитаты синтетические.

## Запрос и режимы

`POST {VITE_API_BASE_URL}/api/analyze`, тело `multipart/form-data`: файлы `before_file` и `after_file`. Заголовок `Content-Type` с boundary устанавливает браузер. UI допускает непустые `.docx`, `.pdf`, `.txt` размером не более `20 * 1024 * 1024` байт каждый; это проверка расширения и размера, не содержимого.

- Только строка `VITE_USE_MOCK=false` включает отправку своих файлов; по умолчанию mock включён. `VITE_API_BASE_URL` по умолчанию пуст: используется текущий origin. Например, `http://localhost:8000` даёт `http://localhost:8000/api/analyze`.
...
type FindingType = 'LOST' | 'MOVED' | 'DUPLICATED' | 'CONFLICT' | 'CREATED'
...
В заключении «Покрытие источниками» — число выводов хотя бы с одной цитатой; `VERIFIED` подсчитывается отдельно. Поскольку валидатор требует источник у каждого вывода, валидный непустой ответ имеет полное покрытие цитатами, но может содержать `NEEDS_REVIEW`. Для интеграционной проверки используйте `npm test`; отдельный рабочий backend проект не предоставляет.
