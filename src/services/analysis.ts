...
const evidence = (value: unknown, document: string, path: string): Evidence[] => array(value, path).map((item, index) => {
  ...
  return {
    ...
    ...(source.clause_id !== undefined ? { clause_id: string(source.clause_id, `${sourcePath}.clause_id`) } : {}),
  }
})
...
function validateViewResponse(value: unknown, coreCounts?: Map<string, number>): AnalysisResult {
  ...
  array(result.findings, 'findings').forEach((item, index) => {
    ...
    if ((coreCounts ? ['MOVED'] : ['MOVED', 'DUPLICATED']).includes(type) && (!beforeEvidence.length || !afterEvidence.length)) fail(path, 'сопоставлению функции требуются источники BEFORE и AFTER')
    if (['DUPLICATED', 'OVERLAP'].includes(type) && !afterEvidence.length) fail(path, 'сопоставлению ответственности требуется источник AFTER')
...
export function validateAnalysisResponse(value: unknown): AnalysisResult {
  ...
  const findings = array(raw.findings, 'findings').map((item, index) => {
    ...
    const beforeSources = evidence(finding.before_evidence, before, `${path}.before_evidence`)
    const afterSources = evidence(finding.after_evidence, after, `${path}.after_evidence`)
  ...
  return validateViewResponse({ ...raw, units, transformations, function_matches: matches, findings }, counts)
}
